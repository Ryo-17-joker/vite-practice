...
const Janken = () => {
  ...
  const playJanken = (event: React.MouseEvent<HTMLButtonElement, MouseEvent>): void => {
    const userHand: number = Number(event.currentTarget.value)
  ...
  return (
    <>
      <h1>ジャンケンアプリ</h1>
      <button onClick={playJanken} value={0}>グー</button>
      <button onClick={playJanken} value={1}>チョキ</button>
      <button onClick={playJanken} value={2}>パー</button>
      <p>出した手{jankenResult.userHand}</p>
      <p>相手の手{jankenResult.computerHand}</p>
      <p>結果一覧</p>
      {jankenResult.results.map((value) => {
        return (
          <p>{value}</p>
        )
      })}
